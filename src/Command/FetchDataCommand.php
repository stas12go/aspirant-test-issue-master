...
class FetchDataCommand extends Command
{
    ...
    private const NUMBER_OF_ENTRIES = 10;
    ...
    //    dd((string) $xml->channel->item[0]->children($namespace)->encoded);
        ...
        for ($key = 0; $key < self::NUMBER_OF_ENTRIES; $key++) {
            $trailer = $this->getMovie((string) $xml->channel->item[$key]->title)
                ->setTitle((string) $xml->channel->item[$key]->title)
                ->setDescription((string) $xml->channel->item[$key]->description)
                ->setLink((string) $xml->channel->item[$key]->link)
                ->setPubDate($this->parseDate((string) $xml->channel->item[$key]->pubDate));
